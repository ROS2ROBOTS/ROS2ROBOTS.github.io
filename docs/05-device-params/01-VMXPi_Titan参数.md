...
为了兼容不同的设备平台，需要根据实际使用需求调整硬件参数。配置文件路径如下，默认示例参数通常可用，但可能与您的硬件平台存在差异，请重点关注独立列出的参数项。
...
📄 **示例参数文件**：[vmxpi_ros2 config](./res/vmxpi_ros2/config.yaml)
...
> 💡 **建议**：若无特殊需求，保持默认值即可；若出现控制延迟或资源不足，优先降低 `update_rate` 和 `vmxpi_update_rate`。
...
> ⚠️ **注意**：修改 CAN ID 或编码器来源前，请确认硬件实际接线与固件配置，否则可能导致电机失控。

---

## 3️⃣ 完整配置示例（一个 Titan 设备）

以下为 `config.yaml` 的完整内容，您可以根据实际硬件平台修改对应字段。
...
> ✅ **验证**：启动后可通过 `ros2 param list` 查看当前运行参数，或观察电机行为确认配置正确。
...
📌 **总结**：合理调整 VMXPi 和 Titan 参数，可以显著提升系统稳定性与响应性能。建议先在空载环境下测试参数变化，再应用到实际任务中。
